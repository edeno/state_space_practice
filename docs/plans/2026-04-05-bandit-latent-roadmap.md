...
- uncertainty-aware behavioral modeling (P2.5)
...
MultinomialChoiceModel (DONE) ──► CovariateChoiceModel (DONE)
 ...
 "How certain is the animal, and
  does uncertainty shape policy?"
        │                                    │
        └────────────┬───────────────────────┘
                     ▼
...
P2 and P3 are independent and answer different questions. P2.5 adds first-class uncertainty summaries and uncertainty-aware policy terms on top of the existing non-switching models, and provides the cleanest bridge from behavioral posteriors to later neural analyses. P4 is the ambitious integration.
...
- **P2.5 (Uncertainty-Aware Behavioral Modeling)** exposes value variance, belief entropy, and predictive uncertainty as first-class behavioral outputs, and adds the first uncertainty-aware policy path before switching models are introduced.
