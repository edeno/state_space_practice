...
- `_sgd_loss_fn(params: dict, *args) → Array` — returns scalar loss (negative marginal LL / n_timesteps). Receives **constrained** parameters.
...
**Breaking change check:** The existing `POSITIVE`, `UNIT_INTERVAL`, `UNCONSTRAINED`, `PSD_MATRIX` constants must be updated to include `trainable=True` as the third field. Verify all existing call sites still work since `trainable=True` is the default.
...
**Critical:** `_build_param_spec` must branch on `self.initial_state_method`:
- `"set_initial_to_zero"`: only `sigma_epsilon` is learnable
- `"user_provided"`: `sigma_epsilon` + `init_learning_state` + `init_learning_variance`
- Other modes: follow existing `_m_step` behavior for what gets updated
...
#### What it learns
- `process_cov` (Q diagonal) — POSITIVE (per-basis-function variance)
- `init_mean` — UNCONSTRAINED
- `init_cov` — PSD_MATRIX

#### Step 4.1: Add mixin, implement four methods

Similar to PointProcessModel but with spatial basis functions.
...
Learnable parameters for SGD:

- `measurement_matrix` (H) — shape `(n_sources, 2*n_oscillators, n_discrete_states)`, UNCONSTRAINED. **This is the main scientifically relevant parameter.**
...
Fixed parameters (frozen via `trainable=False` or excluded):

- `freqs` — fixed at initialization, used to construct A.
- `auto_regressive_coef` — fixed, UNIT_INTERVAL domain constraint (in [0, 1] per `oscillator_utils.py`).
- `process_variance` — fixed, determines diagonal Q.
- `measurement_variance` — fixed, determines scalar R.

Note: The scientific initialization parameters (`freqs`, `auto_regressive_coef`, `process_variance`, `measurement_variance`) are used to construct the fixed A, Q, R matrices at initialization. They are NOT learned during fitting (neither EM nor SGD). If a future use case requires learning dynamics parameters, that would be a different model (or a new flag like `update_continuous_transition_matrix=True`).
...
- `freqs` — POSITIVE
- `auto_regressive_coef` — UNIT_INTERVAL
- `process_variance` — shape `(n_oscillators,)`, POSITIVE (constant across states)
- `measurement_variance` — scalar, POSITIVE
- `phase_difference` — shape `(n_oscillators, n_oscillators, n_discrete_states)`, UNCONSTRAINED. Per-state coupling phase.
- `coupling_strength` — shape `(n_oscillators, n_oscillators, n_discrete_states)`, UNCONSTRAINED. Per-state coupling strength in A.
- Z, init_mean, init_cov, init_prob — same transforms

`_sgd_loss_fn` reconstructs A from `freqs`, `auto_regressive_coef`, `phase_difference`, and `coupling_strength` via `oscillator_utils`, producing per-state transition matrices with valid oscillator structure.
...
`_check_sgd_initialized()` must verify that parameters are allocated AND that the spike GLM parameters (`baseline_`, `spike_weights_`) are initialized. The existing `fit()` path runs warm initialization (spectral clustering of discrete states, placeholder smoother statistics, initial spike M-step) before the main EM loop. For SGD, only parameter allocation is required — the warm-start heuristics are EM-specific. Callers must call `model.initialize_parameters(key=..., obs=...)` before `fit_sgd()`.
